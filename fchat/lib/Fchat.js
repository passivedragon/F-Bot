...
var _FchatBasic = _interopRequireDefault(require("./FchatBasic"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

class Fchat extends _FchatBasic.default {
  ...
  onJCH({
    ...
    if (character.identity === this.user.character) {
      //Client is joining, so add as a new channel
      ...
      //A new character is joining, so add this character to the channel's list of users
  ...
  onLCH({
    ...
    if (chan) {
      if (character === this.user.character) {
        //Client is leaving, so remove the channel
  ...
  onIGN({
    ...
    switch (action) {
      case 'init':
        //Init action: Full ignore list is given
        this.ignoreList = characters;
        break;

      case 'add':
        //Add action: New name is added to the ignore list
      ...
      case 'delete':
        //Delete action: Name is deleted from the ignore list
