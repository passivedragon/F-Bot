...
class FchatBasic {
  ...
  async connect(account, password, character, providedTicket) {
    ...
    try {
      ...
      this.socket = new _ws.default(this.config.fchat.url);
  ...
  async requestTicket(account, password) {
    ...
    form.append("password", password); //Build url from base url and endpoint from config

    var apiConfig = this.config.api;
    var ticketUrl = (apiConfig.apiBaseUrl || "") + apiConfig.endpoints.getApiTicket;
    var json = await (0, _nodeFetch.default)(ticketUrl, {
      method: "POST",
      body: form
    }).then(res => {
      return res.json();
  ...
  socketOnOpen(character, account, ticket) {
    ...
    var client = this.config.fchat.client; //Send an IDN command for identifying with the server, all fields here are required

    this.send('IDN', {
        character,
        cversion: this.version || "0.0.1",
        method: 'ticket',
        account,
        ticket,
        cname: client.name,
  ...
  socketOnMessage(message) {
    ...
    if (message.length > 2) {
      //Messages of less than two characters are ignored
      var command = message.substring(0, 3);
      var data; //If message has a json body (data), retrieve and handle failure case
  ...
  send(command, body) {
    ...
    return false; //no socket, no send!
  }
  ...
  handleCommand(command, data) {
    ...
    switch (command) {
        ...
        case "CIU":
            //Invite to join channel, only auto join if configuration has this option set
        ...
        case "PIN":
            //Ping from server, only auto ping if configuration has this option set
            if (options.autoPing) {
              this.send("PIN");
            }
